...
panda_urdf = main_dir + '/urdf_files/panda.urdf'
...
class PandaRobotEnv_(gym.Env):
    ...
    def __init__(self,
                 ...
                 is_discrete=False,
                 max_steps=7000,  # 7000
                 action_repitions=False,
                 dist_specs=None,
                 # maxDist=0.25,  # old val. 0.08
                 # maxDeviation=0.25,
                 max_error_pos=0.1,
                 target_pos=(0.64, -0.13, 0.79),
                 eval_flag=False,
                 seed=0):

        if dist_specs is None:
            # 0 = Euclidean distance, A = Use improved distance metric
            dist_specs = [0, 'A']

        # Parameter settings
        self._distance_measure_specifications = dist_specs
        self._fixed_nr_action_repetitions = action_repitions
        self._isDiscrete = is_discrete
        self._timeStep = 1. / 240.
        self._action_repetition = action_repetition
        self._isEnableSelfCollision = enable_self_collision
        # self._maxDist = maxDist
        # self._maxDeviation = maxDeviation
        # self._maxErrorPos = maxErrorPos
        self._evalFlag = eval_flag
        self._ray = None
        ...
        self._joint_damping = [0.1] * 9
        self._residual_threshold = 0.002
        self._max_IK_points = 4
        self._max_error_pos = max_error_pos
        # Action boundaries
        self._action_low = np.array([-2, -2, -2])
        self._action_high = np.array([2, 2, 2])
        ...
        self._joint_pos = []
        # Metric used for determination of terminal states
        self._dist_to_obj_primary = 0.0
        # Metric used for determination of terminal states, init. with abritraty large val.
        self._dist_to_target_hold_primary = 0.0
        # Metric used for reward computation
        self._dist_to_obj_secondary = 0.0
        # self._dist_to_obj_secondary translated to a reward
        self._reward_dist = 0.0
        # self._dist_to_target_hold_secondary translated to reward
        self._reward_dist_goal_target = 0.0
        # self._dev_from_goal_vec_secondary translated to a reward
        ...
        # N. controlled joints
        # self._num_joints = min(FRANKA_CONTROLLED_JOINTS, self._p.getNumJoints(self._robot))
        # self._joint_indices_control = FRANKA_JOINT_INDICES_TO_CONTROL

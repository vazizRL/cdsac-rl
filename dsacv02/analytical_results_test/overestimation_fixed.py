...
def compute_delta_c(mu_curr, std_curr, supports, lr, cramer_supp, tar_noisy, tar_true, nabla=1, dev='cpu'):
    """
    ...
    c_tilde = cramer_optim_1k(pdf_target=tar_noisy, pdf_curr=pdf_curr, standard_supp=cramer_supp, n_kernels=1, dev=dev)
    c_star = cramer_optim_1k(pdf_target=tar_true, pdf_curr=pdf_curr, standard_supp=cramer_supp, n_kernels=1, dev=dev)

    frac_tilde = b_tilde / c_tilde
    frac_star = b_star / c_star

    diff_quotient = frac_tilde - frac_star

    frac = -1 / std_curr

    return lr * frac * diff_quotient * nabla
...
if __name__ == '__main__':
    ...
    # Values
    mu_tar_true, std_tar_true = 0.0, 20.0
    mu_tar_high1, std_tar_high1 = 100.0, 20.0
    ...
    for mu_tar_noisy_i, std_tar_i, tar_distr in zip(mu_tar_noisy_lists, std_tar_noisy_lists, target_distributions_noisy):
        ...
        for std_i in std_range:
            delta_i = compute_delta_c(mu_curr=mu_curr_fixed, std_curr=std_i, supports=supp_outer_var_std, lr=learning_rate,

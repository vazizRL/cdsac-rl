...
if __name__ == '__main__':
    ...
    # Target distribution
    mean_tar = (torch.ones(size=(mb_size, 1)) * torch.tensor([2.0, 15.0], dtype=torch.float64)).to(device)
    ...
    '''
    Fit to target with changing stds
    '''
    ...
    ts = time.time()
    save_path = curr_path + '/' + f'VarStd_{std_interval_l}-{std_interval_u}_Mod1_' + str(ts)
    os.mkdir(save_path)

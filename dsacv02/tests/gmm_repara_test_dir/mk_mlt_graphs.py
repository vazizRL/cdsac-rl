...
sub_dir = ''
dir_name = 'Test_Higher_Diff_Mean_mod2_C'
...
cdf_gmm_pred_tar = torch.load(loading_path + '/' + 'cdf_gmm_pred_tar').cpu().numpy()
...
plt.plot(cdf_gmm_pred_tar, label='cdf_gmm_pred_tar')
...
if mod2:
    ...
    '''
    plt.plot(kl_loss, label='KL Loss for target')

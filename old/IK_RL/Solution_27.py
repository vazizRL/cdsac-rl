...
class PandaRobotEnv_(gym.Env):
    ...
    def __init__(self,
        ...
        # Debugging
        self._action_low = np.array([-100])   # old: [-15,-15,-15]
        self._action_high = np.array([100])  # old: [15, 15, 15]
        self._box_boundary_low = np.array([0.3, -0.4, 0.55])  # old: [0.3, 0.35, 0.55]
        self._box_boundary_high = np.array([0.65, 0.85, 1])  # old:  [0.65, 0.85, 1]
        self._delta_x_points = None
        self._delta_y_points = None
        self._delta_z_points = None
    ...
    # Transform one dimenesional input to R3 coordinates
    def action_transformer(self, action):
        point_HKE = abs(-100 - (action))
        point_index = point_HKE // self._step_size

        # x coordinate
        x_stacks = point_index // (self._delta_y_points * self._delta_z_points)
        x_coordinate = self._box_boundary_low[0] + (0.01 * x_stacks)

        # y coordinate
        start_y = point_index // self._delta_y_points
        y_diff = point_index % self._delta_y_points
        if start_y % 2 == 0 or start_y == 0:
            y_coordinate = self._box_boundary_high[1] - (0.01 * y_diff)
        else:
            y_coordinate = self._box_boundary_low[1] + (0.01 * y_diff)

        # z coordinate
        y_stacks = point_index // self._delta_y_points
        start_z = y_stacks // self._delta_z_points
        z_diff = y_stacks % self._delta_z_points
        if start_z % 2 == 0 or start_z == 0:
            z_coordinate = self._box_boundary_high[2] - (z_diff * 0.01)
        else:
            z_coordinate = self._box_boundary_low[2] + (z_diff * 0.01)
    ...
    def reset(self):
        ...
        self._boundary_selected = False
        self._previously_crashless = 0
        self._delta_x_points = (self._box_boundary_high[0] - self._box_boundary_low[0]) * 100
        self._delta_y_points = (self._box_boundary_high[1] - self._box_boundary_low[1]) * 100
        self._delta_z_points = (self._box_boundary_high[2] - self._box_boundary_low[2]) * 100
        p.resetSimulation()
        ...
        random_spawn_prob = random.random()
        xpos = 0.40 + 0.10 * random_spawn_prob
        if random_spawn_prob < 0.5:
            ypos = 0.5 + 0.2 * random.random()
            orn = p.getQuaternionFromEuler([0, 0, (math.pi / 2) - 0.2])
            self._target_pos = [0.64, -0.13, 0.79]
        else:
            ypos = -0.2 + 0.1 * random.random()
            orn = p.getQuaternionFromEuler([0, 0, 0])
            self._target_pos = [0.64, 0.57, 0.79]
        ...
        self._step_size = 200 / ((self._delta_x_points + 1) * (self._delta_y_points + 1) *
                               (self._delta_z_points + 1))                  # old: no number_of_points

        # debug_step_size = 0.008
        # for i in range(int(-100/debug_step_size), int(100/debug_step_size)):
        #     i = i * debug_step_size
        #     self._p.addUserDebugLine(self.action_transformer(i), self.action_transformer(i + debug_step_size))
    ...
    def step(self, action):
        ...
        [x, y, z] = self.action_transformer(action)
        action = [x, y, z]
        ...
        while self._EE_to_IK > self._residual_threshold:
            ...
            if raw_output == self._action_low or raw_output == self._action_high:
            ...
            if self._renders:
                time.sleep(0.05)  # old value 0.05
    ...
    def _reward(self):
        ...
        if self._ray[0][0] == self._blockUid and not self._switch_grip_state:
            reward = 0.5  # old: reward += 1
            self._switch_grip_state = True
...
#
#     # actions = action_start + (action_nr * 0.01)
#     action_nr += 1
#
#     if action_nr == 1:
#         actions = -1.6373791

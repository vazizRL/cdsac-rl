...
class PandaRobotEnv_(gym.Env):
    ...
    def reset(self):
        ...
        self._trayUid = p.loadURDF(os.path.join(self._urdfRoot, "tray/tray.urdf"), 0.500000,
                                   -0.1, 0.63, 0.000000, 0.000000, 1.000000, 0.000000)

        self._trayUid = p.loadURDF(os.path.join(self._urdfRoot, "tray/tray.urdf"), 0.500000,
                                   0.6, 0.63, 0.000000, 0.000000, 1.000000, 0.000000)

        xpos = 0.41 + 0.12 * random.random()
    ...
    def step(self, action):
        ...
        if action_value <= -0.8:
            action = [self._goal_pos[0] - 0.2, self._goal_pos[1], self._goal_pos[2] + 0.2]
        elif action_value >= -0.8 and action_value <= -0.6:
            action = [self._goal_pos[0] -0.15, self._goal_pos[1], self._goal_pos[2] + 0.2]
        elif action_value >= -0.6 and action_value <= -0.4:
            action = [self._goal_pos[0] - 0.10, self._goal_pos[1], self._goal_pos[2] + 0.2]
        elif action_value >= -0.4 and action_value <= -0.2:
            action = [self._goal_pos[0] - 0.05, self._goal_pos[1], self._goal_pos[2] + 0.2]
        elif action_value >= -0.2 and action_value <= 0:
            action = [self._goal_pos[0], self._goal_pos[1], self._goal_pos[2] + 0.2]
        elif action_value >= 0 and action_value <= 0.2:
            action = [self._goal_pos[0] + 0.05, self._goal_pos[1], self._goal_pos[2] + 0.2]
        elif action_value >= 0.2 and action_value <= 0.4:
            action = [self._goal_pos[0] + 0.10, self._goal_pos[1], self._goal_pos[2] + 0.2]
        elif action_value >= 0.4 and action_value <= 0.6:
            action = [self._goal_pos[0] + 0.15, self._goal_pos[1], self._goal_pos[2] + 0.2]
        elif action_value >= 0.6 and action_value <= 0.8:
            action = [self._goal_pos[0] + 0.20, self._goal_pos[1], self._goal_pos[2] + 0.2]
        elif action_value > 0.8:
            action = [self._goal_pos[0] + 0.25, self._goal_pos[1], self._goal_pos[2] + 0.2]
...
#     # elif action_nr == 2:
#     #     actions = [environment._goal_pos[0], environment._goal_pos[1], environment._goal_pos[2] + 0.25]
#     # elif action_nr == 3:

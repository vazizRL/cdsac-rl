...
class PandaRobotEnv_(gym.Env):
    ...
    def __init__(self,
        ...
        self._EE_orn = p.getQuaternionFromEuler([0, -math.pi, 0])
        self._joint_damping = [0.1] * 9 # old 0.1
        self._residual_threshold = 0.002  # old: 0.003
        self._max_IK_points = 8
        self._max_error_pos = maxErrorPos
        # Debugging
        # self.debugReward = 0
        self._action_low = np.array([-2, -2, -2, -math.pi])   # old: [-15,-15,-15]
        self._action_high = np.array([2, 2, 2, math.pi])  # old: [15, 15, 15]
    ...
    def get_joint_config(self, nr_joints=None):
        ...
        return [0.85, 0.8, 0, -1, 0, 1.5, 0.9, 0, 0.5, 0.05, 0.05]
    ...
    def obtain_measurements(self):
        ...
        self._EE_pos, self._EE_orn = self._p.getLinkState(self._robot, 11)[:2]
        self._EE_orn = self._p.getEulerFromQuaternion(self._EE_orn)
        ...
        if self._p.getContactPoints(self._robot):  # and self._p.getContactPoints(6)[0][2] != self._blockUid:
    ...
    def get_reward_measures(self):
        ...
        distance_EE_target_prev = distance.euclidean(self._EE_pos_prev, self._target_pos)
        distance_EE_target = distance.euclidean(self._EE_pos, self._target_pos)
        # Distance Calculations
        self._EE_to_target = distance_EE_target

        self._reward_dist = distance_EE_target_prev - distance_EE_target
    ...
    def reset(self):
        ...
        # Elimintated tables and trays and the block
        # Placed a well behind the robot and an obstacle in the working field

        visualShape = p.createVisualShape(shapeType=p.GEOM_BOX, halfExtents=[0.5, 0.05, 0.6])
        collisionShape = p.createCollisionShape(shapeType=p.GEOM_BOX, halfExtents=[0.5, 0.1, 0.6])

        orn_wall = p.getQuaternionFromEuler([0, 0, math.pi / 2])
        self._wall = p.createMultiBody(baseMass=800,
                                       baseVisualShapeIndex=visualShape,
                                       baseCollisionShapeIndex=collisionShape,
                                       basePosition=[-0.4, 0, 0.6],
                                       baseOrientation=orn_wall)

        orn_obstacle = p.getQuaternionFromEuler([math.pi / 2, 0, -math.pi / 2])
        p.loadURDF("C:/Users/XMG/Desktop/Master/Masterarbeit/Custom_Objects/Obstacle_Refined/obstacle_refined.obj.urdf",
                   [0.68, 0, 0.3], orn_obstacle, useFixedBase=1)


        p.setGravity(0, 0, -9.8)
        self._robot = p.loadURDF(self._robo_path, basePosition=[0, 0.0, 0.3], useFixedBase=1)  # z = 0.6; y =0.25
    ...
    def getExtendedObservation(self):
        ...
        observation = []
        observation.extend(self._EE_orn)  # Euler Angles of the EE
    ...
    def step(self, action):
        ...
        self._EE_to_IK = distance.euclidean(self._EE_pos, action[:3])
        self._current_IK_points += 1

        # self._EE_orn = self._p.getEulerFromQuaternion(self._EE_orn)

        target_orn = list(self._EE_orn)
        target_orn[1] = action[3]
        target_orn = self._p.getQuaternionFromEuler(target_orn)

        while self._EE_to_IK > 0.01 or abs(self._EE_orn[1] - action[3]) > 0.01:

            if self._action_low[0] in action or self._action_low[1] in action or \
                    self._action_low[2] in action or self._action_low[3] in action or \
                    self._action_high[0] in action or self._action_high[1] in action or \
                    self._action_high[2] in action or self._action_high[3] in action:
            ...
            self._EE_to_IK = distance.euclidean(self._EE_pos, action[:3])
            self._EE_orn = self._p.getLinkState(self._robot, 11)[1]
            self._EE_orn = self._p.getEulerFromQuaternion(self._EE_orn)
            self._joint_pos = self._p.calculateInverseKinematics(self._robot, 11, action[:3],
                                                                 target_orn,
            ...
            if ik_step_count_limit > 80:  # old: 160
                break
                # pass
    ...
    def _reward(self):
        ...
        if self._contact_with_env and not self._boundary_selected:
            ...
            reward = (-0.95 + (self._previously_crashless*0.08))                        # old: *(-0.1)
...
#
#     if action_nr == 1:
#         actions = [0.7424035,  0.0812215,  0.62876195, 1.4212569]
#         # actions = [0.7015704326197145, 0.2500000000001144, 1.1637235212338237]
#     elif action_nr == 2:
#         actions = [0.7424035,  0,  0.62876195, 1.4212569]
#     elif action_nr == 3:
#         actions = [0.7424035,  -0.0812215,  0.62876195, 1.4212569]
#     elif action_nr == 4:
#         actions = [0.7424035,  -0.15,  0.62876195, 1.4212569]
#     elif action_nr == 5:
#         actions = [0.7424035,  -0.2,  0.62876195, 1.4212569]
#     elif action_nr == 6:
#         actions = [0.7424035,  -0.25,  0.62876195, 1.4212569]
#     elif action_nr == 7:
#         actions = [0.7424035, -0.35, 0.62876195, 1.4212569]
#     elif action_nr == 8:
#         actions = [0.7424035, -0.55, 0.62876195, 1.4212569]
#
#     else:
#         rand_x = random.uniform(0.25, 0.7)
#         rand_y = random.uniform(-0.2, 0.5)
#         rand_z = random.uniform(0.7, 0.9)
#         actions = [rand_x, rand_y, rand_z]
#

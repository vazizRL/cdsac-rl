...
class PandaRobotEnv_(gym.Env):
    ...
    def __init__(self,
        ...
        # Debugging
        self._action_low = np.array([-1])   # Old: [-15,-15,-15]
        self._action_high = np.array([1])  # Old: [15, 15, 15]
        self._box_boundary_low = np.array([0.3, 0.35, 0.55])  # Old: [0.3, 0.2, 0.55]
        self._box_boundary_high = np.array([0.65, 0.85, 1])  # Old:  [0.65, 0.7, 1]
    ...
    def reset(self):
        ...
        self._step_size = 2 / ((self._delta_x_points + 1) * (self._delta_y_points + 1) *
                               (self._delta_z_points + 1))                  # Old: no number_of_points
        # debug_step = 0.0001
        # for i in range(int(-1/debug_step), int(1/debug_step)):
        #     i = i * debug_step
        #     self._p.addUserDebugLine(self.action_transformer(i), self.action_transformer(i + debug_step))
...
#
#     # actions = action_start + (action_nr * 0.0001)
#     action_nr += 1
#
#     if action_nr == 1:
#         actions = -1

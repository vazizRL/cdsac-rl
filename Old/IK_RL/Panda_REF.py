...
class PandaRobotEnv_(gym.Env):
    ...
    def __init__(self,
                 ...
                 maxDeviation=0.25,
                 maxErrorPos=0.1,
                 targetPos=[0.64, -0.1, 0.79],
                 evalFlag=False):

        if distSpecifications is None:
            distSpecifications = [0, 'B']  # 0 = Euclidean distance, A = Use improved distance metric
    ...
    def get_joint_config(self, nr_joints=None):
        ...
        # exploitation
        return [0, 0.65, 0, -1, 0, 1.5, 0.9, 0, 0, 1, 1]
    ...
    def get_contact_and_target_dist(self):
        ...
        if self.contact_info_l and self.contact_info_l[0][9] >= 10 and self.contact_info_r and self.contact_info_r[0][
            9] >= 10:
    ...
    def reset(self):
        ...
        self._table = p.loadURDF(os.path.join(self._urdfRoot, "table/table.urdf"), 0.5000000, 0.00000, 0,
        ...
        self.blockUid = p.loadURDF(os.path.join(self._urdfRoot, "block.urdf"), xpos, ypos, 0.67,
    ...
    def step(self, action):
        ...
        # if self._p.getContactPoints(6):
        #     print(self._p.getContactPoints(6)[0][2])
        ...
        # jointPos = [jointPos[i] + np.clip(action[i], -0.5, 0.5) for i in range(len(self._joint_indices_control))]
        ...
        self._p.setJointMotorControlArray(bodyUniqueId=self._robot,
                                          ...
                                          forces=[100] * len(self._joint_indices_control))
...
param3 = p.addUserDebugParameter("a3", -0.5, 0.5, 0.0)
